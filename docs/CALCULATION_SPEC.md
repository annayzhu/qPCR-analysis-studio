...
同一样本/基因跨板出现时，只汇总实际具有完整同板内参的板段。完整结果中的目标/内参 mean、SD 与 SEM 从这些板段内所有实际参与的单孔复孔汇总，并由同一组 mean 得到 ΔCq 与归一化量。合并 SD 使用组内与组间平方和及总复孔数的 `n−1` 分母，SEM 始终为合并 SD 除以总有效复孔数平方根，不对各板 SD/SEM 做简单平均。
...
校准样本本身是数值 1 的定义锚点，但其技术测量不确定性不能强制记为 0：校准样本保留自身目标与内参技术复孔围绕各自均值形成的 ΔCq SD/SEM，并将其传播到相对表达量。非校准样本则进一步将自身与校准样本的 ΔCq 不确定性按四分量相加后开方传播。目标或任一内参只有 1 个有效技术复孔时，相应 SD/SEM 记为 `null` 且不绘制误差线；不得把缺失 SD/SEM 当作 0。每个图自动列出目标及各内参实际参与计算的有效复孔数。
